...
class AllocineSpider(scrapy.Spider):
    ...
    def parse_manga(self, response):
        ...
        for film in liste_film:
            ...
            try:
                item['genre'] = '-'.join(film.css('div div div div.meta-body-info span::text')[1:].extract()).strip()
            except:
                 item['genre'] = 'None'

            # Score du film
            try:
                item['score'] = (float(film.css('div div div div.rating-item-content div span.stareval-note::text')[0].extract()[0].strip()) + float(film.css('div div div div.rating-item-content div span.stareval-note::text')[1].extract()[0].strip())) / 2
            except:
                item['score'] = 'None'
